...
file_path = "C:/Course Materials/COMP3850/Blender/Sample Images/1. Helix Nebula @ w = 4844.tiff"
...
#----------- change the pixels of the image to capture saturated stars -----------
# Create a copy of the pixel data
pixels = list(image.pixels)

# Define threshold
threshold_value = 0.95 # You can change this value to whatever threshold you want

# Iterate through the pixels in blocks of 4 (R, G, B, A)
for i in range(0, len(pixels), 4):
    # If the pixel value of the chosen channel is greater than the threshold, set it to pink
    if pixels[i + channel_idx] > threshold_value:
        pixels[i] = 1.0    # Red
        pixels[i + 1] = 0.0  # Green
        pixels[i + 2] = 1.0  # Blue
